...
func TestRunTUIWiresJobTools(t *testing.T) {
	...
	defer st.Close()
	reg := buildRegistry(t.TempDir(), st, jobmgr.NewManager(st, nil), llm.NewModelRegistry(config.DefaultCatalog()), config.DefaultConfig(), newTestInstaller(t), skills.NewLoader(nil))
