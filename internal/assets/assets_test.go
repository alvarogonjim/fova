...
func TestLoadMaterializesAndParses(t *testing.T) {
	...
	if len(b.Skills) != 15 {
		t.Errorf("want 15 built-in skills, got %d", len(b.Skills))
	}
	if b.SystemPrompt == "" {
...
func TestLoadReportsBadSkillButKeepsGoing(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("FOVA_CONFIG_DIR", dir)
	if _, err := Load(); err != nil { // first run materializes the 15 built-ins
		t.Fatal(err)
	}
	...
	if len(b.Skills) != 15 {
		t.Errorf("the 15 good skills should still load, got %d", len(b.Skills))
	}
	if b.Report.OK() {
