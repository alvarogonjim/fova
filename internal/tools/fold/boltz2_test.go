...
// TestBoltz2ValidateRejectsInvalidJSON exercises the malformed-bytes branch
// of Validate: the editable gate must surface a clear "invalid JSON" error so
// the user sees an editor reopen with a fix-it hint rather than a stack trace.
func TestBoltz2ValidateRejectsInvalidJSON(t *testing.T) {
	tool := NewBoltz2("/ws", nil, nil)
...
// TestBoltz2ValidatePassesGoodInput is the happy path: a minimal valid
// single-protein request must return nil so the editable gate accepts the
// user's edit without re-opening the editor.
func TestBoltz2ValidatePassesGoodInput(t *testing.T) {
	tool := NewBoltz2("/ws", nil, nil)
...
// TestBoltz2ValidateSurfacesPreflightError feeds a request that parses cleanly
// but trips preflight (invalid amino acid). The Validator must return the
// preflight error verbatim so the editable gate pins the same diagnostic the
// user would see from Execute.
func TestBoltz2ValidateSurfacesPreflightError(t *testing.T) {
	tool := NewBoltz2("/ws", nil, nil)
	// '1' is not an amino acid — preflightBoltz2 emits an "invalid residue"
	// diagnostic that the editable gate must propagate unchanged.
	input := json.RawMessage(`{"entities":[{"type":"protein","id":"A","sequence":"MKQ1"}]}`)
	err := tool.Validate(input)
	if err == nil {
		t.Fatal("Validate must surface preflight errors")
	}
	if !strings.Contains(err.Error(), "invalid residue") {
		t.Errorf("error %q must contain the preflight diagnostic \"invalid residue\"", err)
	}
}
